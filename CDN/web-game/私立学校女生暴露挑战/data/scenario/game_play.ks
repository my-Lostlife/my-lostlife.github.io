...
[if exp="f.stage == 'park_night' || f.stage == 'residential_night' || f.stage == 'school_night' || f.stage == 'mall_night'"]
	[playbgm storage="VoiceBack.mp3"]
[elsif exp="f.stage == 'park_morning' || f.stage == 'residential_morning' || f.stage == 'school_morning' || f.stage == 'mall_morning'"]
	[playbgm storage="VoiceBack.mp3"]
[endif]
...
[if exp="f.costume == 'uniform'"]
	[locate x=857 y=511]
	[button graphic="button/command/button_pussy_show.png" storage="game_play.ks" target="pussy_show"]
[else]
...
[if exp="f.costume == 'uniform'"]
	[locate x=770 y=511]
	[button graphic="button/command/button_boobs_show.png" storage="game_play.ks" target="boobs_show"]
[else]
...
[if exp="f.costume == 'coat'"]
	[locate x=770 y=377]
	[button graphic="button/command/button_coat.png" storage="game_play.ks" target="coat_expose"]
[else]
...
[if exp="f.expose_rank >= 2 || f.complete == true"]
	[locate x=857 y=444]
	[button graphic="button/command/button_knead.png" storage="game_play.ks" target="boobs_knead"]
[elsif exp="f.expose_rank < 2"]
...
[if exp="f.expose_rank == 3 || f.complete == true"]
	[locate x=770 y=444]
	[button graphic="button/command/button_masturbation.png" storage="game_play.ks" target="masturbation"]
[elsif exp="f.expose_rank < 3"]
...
[if exp="f.expose_rank == 3 || f.complete == true"]
	[locate x=857 y=377]
	[button graphic="button/command/button_pee.png" storage="game_play.ks" target="pee" exp="f.pee_only = true"]
[elsif exp="f.expose_rank < 3"]
	[image name="pe_gray" layer=0 storage="button_pee_gray.png" x=857 y=377]
[endif]

[locate x=762 y=589]
[button graphic="button/command/button_hide.png" storage="game_play.ks" target="hide"]

[locate x=15 y=567]
[button graphic="button/command/button_stop.png"  storage="game_play.ks" target="game_stop"]
...
[if exp="f.expose_rank == 3 || f.complete == true"]
	[locate x=857 y=377]
	[button graphic="button/command/button_pee.png" storage="game_play.ks" target="pee"]
[elsif exp="f.expose_rank < 3"]
	[image name="pe_gray" layer=0 storage="button_pee_gray.png" x=857 y=377]
[endif]
...
;------------------------------------------------------------------------------------------------------
;待機H
[if exp="f.stage == 'park_morning' || f.stage == 'residential_morning' || f.stage == 'school_morning' || f.stage == 'mall_morning'"]
	[iscript]
...
;見つかった時の処理
[playse buf=1 storage="Hit03-1.mp3" ]
[wait time="100"]
[eval exp="f.life -= 1"]
[eval exp="f.wait_bonus = 0"]
...
[playse buf=1 storage="Shortbridge15-1.mp3"]
...
[free name="command_frame" layer = 0]
[image name="black_screen" layer="0" storage="black.png"]
[image layer="0" storage="game_clear.png" time=1000]
...
[if exp="f.stage == 'park_night'"]
	[if exp="f.park_night_clear == false"]
		[eval exp="f.park_night_clear = true"]
	[endif]
[elsif exp="f.stage == 'residential_night'"]
	[if exp="f.residential_night_clear == false"]
		[eval exp="f.residential_night_clear = true"]
	[endif]
[elsif exp="f.stage == 'school_night'"]
	[if exp="f.school_night_clear == false"]
		[eval exp="f.school_night_clear = true"]
	[endif]
[elsif exp="f.stage == 'mall_night'"]
	[if exp="f.mall_night_clear == false"]
		[eval exp="f.mall_night_clear = true"]
	[endif]
[elsif exp="f.stage == 'park_morning'"]
	[if exp="f.park_morning_clear == false"]
		[eval exp="f.park_morning_clear = true"]
	[endif]
[elsif exp="f.stage == 'residential_morning'"]
	[if exp="f.residential_morning_clear == false"]
		[eval exp="f.residential_morning_clear = true"]
	[endif]
[elsif exp="f.stage == 'school_morning'"]
	[if exp="f.school_morning_clear == false"]
		[eval exp="f.school_morning_clear = true"]
	[endif]
[elsif exp="f.stage == 'mall_morning'"]
	[if exp="f.mall_morning_clear == false"]
		[eval exp="f.mall_morning_clear = true"]
	[endif]
[endif]
...
[if exp="f.complete == false"]
	;処女→露出狂
	[if exp="f.expose_rank == 1"]
		[if exp="f.park_night_clear == true && f.residential_night_clear == true && f.school_night_clear == true && f.mall_night_clear == true"]
			[playse buf=1 storage="レベルが上がったり何かをクリアした時の短いジングル.mp3"]
			[eval exp="f.expose_rank = 2"]
			[locate x=0 y=0]
			[button graphic="button/rank_up_1.png" target="clear_button"]
			[s]
		[endif]

	;露出狂→処女
	[elsif exp="f.expose_rank == 2"]
		[if exp="f.park_morning_clear == true && f.residential_morning_clear == true && f.school_morning_clear == true && f.mall_morning_clear == true"]
			[playse buf=1 storage="レベルが上がったり何かをクリアした時の短いジングル.mp3"]
			[eval exp="f.expose_rank = 3"]
			[eval exp="f.complete = true"]
			[locate x=0 y=0]
			[button graphic="button/rank_up_2.png" target="clear_button"]
			[s]
		[endif]
	[endif]
[endif]
...
[cm]
[locate x=279 y=400]
[button graphic="button/pose/button_onemore.png" target="restart"]
[locate x=279 y=500]
[button graphic="button/pose/button_home.png" target="home_return"]
[s]
...
[playse buf=1 storage="Onoma-Negative04-1.mp3"]
...
[free name="command_frame" layer = 0]
[image name="black_screen" layer="0" storage="black.png"]
[image layer="0" storage="game_over.png" time=1000]
